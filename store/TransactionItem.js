Ext.define('MovieApp.store.TransactionItem', {
    extend: 'Ext.data.Store',
    alias: 'mrtransactionitem',

    requires:[
        'MovieApp.model.TransactionItem'
    ],

    model: 'MovieApp.model.TransactionItem',
    storeId: 'TransactionItem',
    pageSize:50,
    batchActions: true,
    proxy: {
        type: 'rest',
        api: {
            read: './MovieRental/api/Transaction/get', 
            update: './MovieRental/api/Transaction/put',
            create: './MovieRental/api/Transaction/post',
            destroy: './MovieRental/api/Transaction/delete'
        },
        reader: {
            type: 'json',
            rootProperty: 'data',
            messageProperty: 'message'
        },
        writer: {
            type: 'json',
            allowSingle: false
        }
    }

});