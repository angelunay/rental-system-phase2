/*
 * File: app/view/TransactionItemViewModel.js
 *
 * This file was generated by Sencha Architect version 4.3.4.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 7.7.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 7.7.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('MovieApp.view.TransactionItemViewModel', {
    extend: 'Ext.app.ViewModel',
    alias: 'viewmodel.mrtransactionitem',

    requires: [
        'MovieApp.store.BufferedRental',
        'MovieApp.store.BufferedMovie'
    ],
    stores: {
                
        name: {
            type:'mrbufferedrental'
        },
        title: {
            type:'mrbufferedmovie'
        },

        status: {
            autoLoad: true,
            data: [
                {strStatus: 'Returned'},
                {strStatus: 'Pending'},
            ],
            fields: [
                {name: 'strStatus'}
            ]
        },
        transacStatus: {
            autoLoad: true,
            data: [
                {strStatus: 'Complete'},
                {strStatus: 'Pending'},
            ],
            fields: [
                {name: 'strStatus'}
            ]
        }
    }
});